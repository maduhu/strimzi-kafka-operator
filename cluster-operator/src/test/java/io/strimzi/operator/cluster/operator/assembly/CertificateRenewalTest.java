...
public class CertificateRenewalTest {
    ...
    private ArgumentCaptor<Secret> reconcileCa(TestContext context, TlsCertificates tlsCertificates, Secret initialSecret) {
        SecretOperator secretOps = mock(SecretOperator.class);

        when(secretOps.get(eq(NAMESPACE), eq(AbstractModel.getClusterCaName(NAME)))).thenReturn(
                initialSecret
        );
        ...
        KafkaAssemblyOperator op = new KafkaAssemblyOperator(vertx, false, 1L, certManager,
                new ResourceOperatorSupplier(null, null, null,
                        null, secretOps, null, null, null, null, null, null));
        Reconciliation reconciliation = new Reconciliation("test-trigger", ResourceType.KAFKA, NAMESPACE, NAME);

        Async async = context.async();
        op.reconcileClusterCa(reconciliation, Labels.EMPTY, tlsCertificates, ar -> {
